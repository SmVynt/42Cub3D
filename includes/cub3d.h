...
/*   Updated: 2025/11/11 21:41:06 by psmolin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline void	put_pixel(mlx_image_t *image, uint32_t x,
			uint32_t y, uint32_t color)
{
	if (x < image->width && y < image->height)
		((uint32_t *)image->pixels)[y * image->width + x] = color;
}
void		draw_line_ray(mlx_image_t *image, t_point p0,
				t_vec2 lookdir, t_map map, int x);
...
void		draw_circle(mlx_image_t *image, t_point center,
				int radius, uint32_t color);
t_vec2		get_ray_end(t_rayrender *ray, t_vec2 start, t_vec2 dir,
				int max_iter, t_direction *wall_dir);
